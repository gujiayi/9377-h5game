...
var H9377 = {
	...
	createCode:function(){
		H9377.getImg('validateCode')
	},
	createCode1:function(){
		H9377.getImg('validateCode1')

	},
	Verification :function(ele,code) {
			...
			if(tx.length>=4){
				getJson(param, function(result){
						if(result.errno!=0){
							...
							if(code=='H9377.VerCode1'){
								H9377.VerCode1=true
							}else{
								H9377.VerCode=true
							}
						}
				}, '/csi/ImageVerify/checkImageCode','POST');
			}
		})
	},
	...
	//@todo 20181105 加上传递渠道参数，适合android数据统计
	channel:$_GET['referer'] ? $_GET['referer'] : 'wvw.qiyouworld.com',
			...
			getJson(param, function(result){
				if (result.msg == '注册成功, 正在为您跳转' || result.msg == '登录成功') {
					...
					if (mod == 'onekey') {
						...
						// 	}
						// }, 1000);
						$('.mod-reg-onekey .btn-enter').click(function() {
							// clearInterval(cdinter);
							regJump();
						});
						$('.mod-reg-onekey .icon-back').click(function(){
							regJump();
						})
					}else{
						regJump();
	...
	createAccount : function(){
		...
		var str1 = 'abcdefghijklmnopqrxtuvwxyz',
			...
			now = new Date;
		myacc.account = 'qy'+ randomNum(10000000,99999999);
...
H9377.init();
H9377.Verification($('#text'),'H9377.VerCode');
H9377.Verification($('#text1'),'H9377.VerCode1');
var CSRegular = {
	uname : function(v){
